...
sex_lbpp <- clin_df |> filter(group == "LBP+") |> tabyl(Demo_Gender) |> adorn_rounding(digits = 2) |> mutate(value = paste0(n, " (", percent * 100, " %)"))
sex_lbpm <- clin_df |> filter(group == "LBP-") |> tabyl(Demo_Gender) |> adorn_rounding(digits = 2) |> mutate(value = paste0(n, " (", percent * 100, " %)"))
pain_duration_cat_lbpp <- clin_df |> filter(group == "LBP+") |> tabyl(pain_duration_cat) |> adorn_rounding(digits = 2) |> mutate(value = paste0(n, " (", percent * 100, " %)"))
...
dunn_test(clin_df, Demo_BMI ~ group, p.adjust.method = "fdr")
...
dunn_test(clin_df, Quest_ODI_Score ~ group, p.adjust.method = "fdr")
...
tukey_hsd(clin_df, Quest_TSK_Score ~ group, p.adjust.method = "fdr")
...
dunn_test(clin_df, Quest_CSI_Score ~ group, p.adjust.method = "fdr")
...
dunn_test(clin_df, Quest_PROMIS_Anxiety ~ group, p.adjust.method = "fdr")
...
dunn_test(clin_df, Quest_PROMIS_Depression ~ group, p.adjust.method = "fdr")
...
blinding <- clin_df |> 
  mutate(blinding = case_when(
    guess_pain == "pain" & str_detect(group, "LBP") ~ T,
    guess_pain == "no pain" & group == "CG" ~ T,
    .default = FALSE
  )) |> 
  summarise(guessed_correctly_n = sum(blinding))
